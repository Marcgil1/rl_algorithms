...
class VPGAgent:
    ...
    def train_on_batch(self, obss, acts, rets):
        self.policy.train_on_batch(
            obss,
            np.concatenate([acts[:, None], rets[:, None]], axis=-1)
        )
    ...
    def _policy_loss(self, acts_and_rews, logits):
        acts, rews = tf.split(acts_and_rews, 2, axis=-1)
